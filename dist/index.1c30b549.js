...
const setRed = (i)=>{
    document.querySelector(`.status_count_item--${i}`).className = `status_count_item status_count_item--${i} red-bg`;
};
const setGreen = (i)=>{
    document.querySelector(`.status_count_item--${i}`).className = `status_count_item status_count_item--${i} green-bg`;
};
const removeBg = (i)=>{
    document.querySelector(`.status_count_item--${i}`).className = `status_count_item status_count_item--${i}`;
};
const initQuestions = ()=>{
    current = 1;
    generateButtons();
...
const getCurrentQuestion = ()=>{
    ...
    if (current === questions.length) nextBtn.className = "question_action_next disabled";
    else nextBtn.className = "question_action_next";
};
...
const counterLoop = ()=>{
    ...
    setTimeout(()=>{
        if (timer === 0) {
            if (current === questions.length) submitQuiz();
            else {
                nextQuestion();
                counterLoop();
            }
...
//# sourceMappingURL=index.1c30b549.js.map
